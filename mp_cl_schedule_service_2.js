...
define(['N/email', 'N/runtime', 'N/search', 'N/record', 'N/http', 'N/log', 'N/error', 'N/url', 'N/format', 'N/currentRecord'],
    function(email, runtime, search, record, http, log, error, url, format, currentRecord) {
        ...
        function onclick_back() {
            ...
            var params = {
                zee: parseInt(currRec.getValue({ fieldId:'zee'})),
                serviceid: currRec.getValue({ fieldId:'service_id'}),
                scriptid: currRec.getValue({ fieldId:'custpage_suitlet'}),
                deployid: currRec.getValue({ fieldId:'custpage_deploy'})
            }
            params = JSON.stringify(params);
            console.log(currRec.getValue({ fieldId:'custpage_suitlet'}));
            console.log(currRec.getValue({ fieldId:'custpage_deploy'}));
            var upload_url = baseURL + url.resolveScript({
                deploymentId: currRec.getValue({ fieldId:'custpage_deploy'}),
                scriptId: currRec.getValue({ fieldId:'custpage_suitlet'})
            }) + '&unlayered=T&service_id=' + currRec.getValue({ fieldId:'service_id'}) + '&custparam_params=' + params;
        ...
        function jQuery(){
            var transfer_stop_linked = currRec.getValue({ fieldId:'custpage_transfer_stop_linked'});
            transfer_stop_linked = transfer_stop_linked.split(',');
            var transfer_type = currRec.getValue({ fieldId:'custpage_transfer_type'});
        ...
        function saveRecord() {
            ...
            var customer_id = currRec.getValue({ fieldId:'customer_id'});
            var service_id = currRec.getValue({ fieldId:'service_id'});
            var stops_ids = currRec.getValue({ fieldId:'stop_ids'});
                        ...
                        if (rows.length == 1 || rows.length == 0) {
                            ...
                            if (isNullorEmpty(latest_time)) {
                                ...
                                if (freq_change == true || (run != old_run) || (service_time != old_service_time) || (earliest_time != old_earliest_time) || (latest_time != old_latest_time)) {
                                    ...
                                    if ($('#adhoc').is(':checked')) {
                                        freq_record.setValue({ fieldId: 'custrecord_service_freq_day_adhoc', value: 'T'});
                                    } else {
                                        freq_record.setValue({ fieldId: 'custrecord_service_freq_day_adhoc', value: 'F'});
                                    }
                                    freq_record.save({
                                        enableSourcing: true,
