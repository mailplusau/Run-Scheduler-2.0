...
define(['N/error', 'N/runtime', 'N/search', 'N/url', 'N/record', 'N/format', 'N/email', 'N/currentRecord'],
    function(error, runtime, search, url, record, format, email, currentRecord) {
        ...
        function pageInit() {
            ...
            var zee_string = currentScript.getValue({
                fieldId: 'zee',
            });

            if (!isNullorEmpty(zee_string)) {
                zee_array = zee_string.split(',');
                zee_text_array = currentScript.getValue({
                    fieldId: ('zee_text').split(','),
                });
                
            }

            if (zee_array.length != 0) {
                op_array = currentScript.getValue({
                    fieldId: ('op').split(','),
                });
                
                run_array = currentScript.getValue({
                    fieldId: ('run').split(','),
                });
                
                optimize_array = currentScript.getValue({
                    fieldId: ('optimisation').split(','),
                });
                
                day = parseInt(currentScript.getValue({fieldId: ('day').split(',')}));
                
                before_time = currentScript.getValue({
                    fieldId: 'beforetime',
                });
                
                after_time = currentScript.getValue({
                    fieldId: 'aftertime',
                });
        ...
        function getRunJSON(zee, run, day, before_time, after_time) {
           ...
           if (!isNullorEmpty(before_time)) {
                serviceLegSearch.filters.push(search.createFilter({
                    ...
                    values: onTimeChange(before_time)
                }));
            }
            if (!isNullorEmpty(after_time)) {
                serviceLegSearch.filters.push(search.createFilter({
                    ...
                    values: onTimeChange(after_time)
                }));
            }
